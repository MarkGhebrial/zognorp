...
pub fn solve_sudoku(grid: &Grid) -> Option<Grid> {
    ...
    // Find the unset cell with the fewest amount of possibilities
    let (best_cell_index, best_cell_possibilities): (usize, &CellPossibilities) = grid
        .possibilities()
        .iter()
        .enumerate()
    ...
    // Try every possible value for the cell with the fewest possibilities
